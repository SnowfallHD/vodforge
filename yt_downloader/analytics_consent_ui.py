...
class AnalyticsConsentPanel:
    def __init__(
        ...
        for index, text in enumerate(
            ...
            stroke = {"fill": THEME["accent"], "width": 2}
            if index == 0:
                icon.create_line(
        ...
        for index, button in enumerate(self.controls):
            button.bind("<Tab>", lambda _e, i=index: self.focus_control((i + 1) % 3))
            button.bind(
                "<Shift-Tab>", lambda _e, i=index: self.focus_control((i - 1) % 3)
            )
            button.bind("<Escape>", lambda _e: self.finish(False))
