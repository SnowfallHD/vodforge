...
| Library item details, output details, source variants, notes/tags/category | `library_detail_layout.py`, `library_scene_facts.py`, `library_annotation_ui.py` | Qt detail reads shared source/output facts and saved annotations. Description now has a shared-control editor and saves through the existing annotation owner with a current-detail identity guard; notes/tags/category use the existing organization popup. Saved-version selector, direct tag chip editing and complete detail actions remain open. |
