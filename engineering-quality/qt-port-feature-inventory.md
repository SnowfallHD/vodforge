...
| Folders, All media, Runs & previews, locations, paging and saved export versions | `archive_browser.py`, `archive_browser_ui.py`, `archive_library_ui.py` | Qt now reads the existing metadata-only `ArchiveBrowserModel` for all three modes and location navigation, with a 48-item page and root recent-export cards. A grouped MP4/MP3 source test selects both versions and returns to the same folder; 1100×740 Qt source folder capture was inspected and 820×560 AX navigation bounds pass. Activity activation, folder selection/relink/recovery, native package behavior and exact card geometry remain open. |
