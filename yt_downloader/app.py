...
def _output_artifact_plan_mismatches(
    probe_data: dict[str, Any],
    plan: ExportPlan | AudioExportPlan,
    *,
    sidecar_summary: dict[str, Any] | None = None,
    ...
    def stream_kbps(stream: dict[str, Any], *, allow_format_fallback: bool = False) -> float | None:
        value = stream.get("bit_rate")
        if value in {None, ""} and allow_format_fallback:
    ...
    if isinstance(plan, AudioExportPlan):
        ...
        if require_sidecar and str(summary.get("Target audio bitrate") or "") != f"{plan.audio_bitrate_kbps} kbps":
        ...
        if measured_kbps is None or not close_numeric(measured_kbps, plan.audio_bitrate_kbps, relative=0.12):
            mismatches.append(f"the measured audio bitrate does not match {plan.audio_bitrate_kbps} kbps")
        if plan.output_sample_rate and not exact_numeric(
            audio.get("sample_rate"),
            plan.output_sample_rate,
        ):
            mismatches.append(f"the audio sample rate does not match {plan.output_sample_rate} Hz")
        if plan.output_channels and not exact_numeric(
            audio.get("channels"),
            plan.output_channels,
        ):
            mismatches.append(f"the audio channel count does not match {plan.output_channels}")
        return mismatches

    if require_sidecar and str(summary.get("Output rate-control mode") or "") != plan.mode.value:
        mismatches.append("the saved video rate-control mode does not match")
    if require_sidecar and str(summary.get("Target video bitrate") or "") != f"{plan.video_bitrate_kbps} kbps":
    ...
    if str(audio.get("codec_name") or "").lower() != plan.output_audio_codec.ffprobe_codec:
        mismatches.append(f"the output audio codec is not {plan.output_audio_codec.ffprobe_codec.upper()}")
    if require_sidecar:
    ...
    if measured_video_kbps is None or not close_numeric(
        ...
        mismatches.append(f"the measured video bitrate does not match {plan.video_bitrate_kbps} kbps")
    ...
    if measured_audio_kbps is not None:
        if plan.output_audio_codec is ManualAudioCodec.MP3:
            ...
            audio_bitrate_matches = target_kbps * 0.40 <= measured_audio_kbps <= target_kbps * 1.25
    ...
    if plan.audio_sample_rate and not exact_numeric(
        ...
        mismatches.append(f"the audio sample rate does not match {plan.audio_sample_rate} Hz")
    if plan.audio_channels and not exact_numeric(
        audio.get("channels"),
        plan.audio_channels,
    ):
        mismatches.append(f"the audio channel count does not match {plan.audio_channels}")
    return mismatches
...
def validate_output_artifact(
    ...
    *,
    ...
    if output_type == OutputType.MP3:
        if isinstance(plan, ExportPlan):
            raise RuntimeError("the MP3 output was validated against an incompatible MP4 export plan")
        ...
        if plan is not None:
            mismatches = _output_artifact_plan_mismatches(data, plan)
            if mismatches:
                raise RuntimeError(f"the MP3 output does not match its export plan: {'; '.join(mismatches)}")
        return data

    if isinstance(plan, AudioExportPlan):
        raise RuntimeError("the MP4 output was validated against an incompatible audio export plan")
    ...
    if plan is not None:
        mismatches = _output_artifact_plan_mismatches(data, plan)
        if mismatches:
            raise RuntimeError(f"the MP4 output does not match its export plan: {'; '.join(mismatches)}")
    return data
