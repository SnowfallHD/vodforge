...
| Library item details, output details, source variants, notes/tags/category | `library_detail_layout.py`, `library_scene_facts.py`, `library_annotation_ui.py` | Qt detail reads shared source/output facts and saved annotations. Description and direct tag chips/addition now save through the existing annotation owner with a current-detail identity guard; notes/category also use the organization popup. The current 1100×740 detail source capture was inspected after both editors. Saved-version selector and complete detail actions remain open. |
