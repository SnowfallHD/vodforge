...
def test_scrollable_action_dialogs_use_the_shared_sleek_scrollbar() -> None:
...
def test_player_icons_are_real_bundled_assets_with_visible_alpha() -> None:
    icon_root = Path(__file__).resolve().parents[1] / "assets" / "icons" / "lucide"

    for name in ("play", "pause", "volume-2"):
        icon_path = icon_root / f"{name}.png"
