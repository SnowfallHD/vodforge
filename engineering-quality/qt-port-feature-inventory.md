...
| Playback controls, volume, chapters, heatmap, preview moments, details, window modes | `media_player_ui.py`, `player_scene_ui.py`, `player_presentation_ui.py`, `player_related.py` | `PlayerScene.qml` keeps the current control material and per-open provider generation, and reads the shared Up Next/recent plan, identity, annotations and source/output facts. It exposes Edit your details and related Play/Details actions. Five midpoint thumbnails use the existing offline `MediaPreviewOwner` through a bounded Qt session; replacement retires prior results and moment buttons use shared seek. Fullscreen/floating/fit/fill now switch the one Qt `MediaPlayer` between embedded and separate-window `VideoOutput` surfaces. Source QML tests verify output rebinding in all three modes; actual video, close/queue/native presentation behavior and captions still need package proof. Complete telemetry parity remains open. |
